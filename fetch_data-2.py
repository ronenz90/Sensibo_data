...
def merge_energy(existing, device_id, new_daily):
    """Merge daily energy — never overwrite real data with zeros."""
    device_energy = existing.get(device_id, {})
    for day, vals in new_daily.items():
        new_kwh = round(vals["kwh"], 3)
        # Skip if we would overwrite real data with zero
        if new_kwh == 0 and device_energy.get(day, {}).get("kwh", 0) > 0:
            continue
        device_energy[day] = {
            "kwh":        new_kwh,
...
def main():
    ...
    for device in devices:
        ...
        # Energy
        try:
            events    = get_ac_events(device_id, days_back=7)
            print(f"  AC events fetched: {len(events)}")
            daily_kwh = compute_daily_energy(events, power_kw, price_per_kwh, days_back=7)
